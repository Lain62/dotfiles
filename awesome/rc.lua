...
-- Widgets for bar
local widgets_t = require("widgets")
local widgets = widgets_t:new()
...
-- Double click function stolen from ilovecookiee on reddit
...
mytextclock:set_format("%A, %Y-%m-%d %H:%M:%S")
...
awful.screen.connect_for_each_screen(function(s)
      ...
      -- s.mywibox:ajust_ratio(1, 0.25, 0.50, 0.25)
      -- s.mywibox_top = awful.wibar({ position = "top", screen = s, height = 24})
...
globalkeys = gears.table.join(
   awful.key({modkey}, "Tab", function() awful.menu.client_list({ theme = { width = 600 } }) end,
   { description = "Open clients list", group="client"}),
   ...
   awful.key({ modkey }, "d", function() awful.util.spawn("rofi -show drun") end,
      {description = "show the menubar", group = "launcher"}),
   -- Screenshots
   awful.key({}, "Print", function() screenshot.area() end,
      {description = "Screenshot an area", group = "awesome"}),
   awful.key({ modkey }, "Print", function() screenshot.fullscreen() end,
      {description = "Screenshot the screen", group = "awesome"}),
   awful.key({ "Shift" }, "Print", function() screenshot.window() end,
      {description = "Screenshot a window", group = "awesome"}),
   -- Pipewire functions
   awful.key({}, "XF86AudioRaiseVolume", function() awful.spawn.with_shell("wpctl set-volume @DEFAULT_AUDIO_SINK@ 0.1+") end, nil),
   awful.key({}, "XF86AudioLowerVolume", function() awful.spawn.with_shell("wpctl set-volume @DEFAULT_AUDIO_SINK@ 0.1-") end, nil),
   awful.key({}, "XF86AudioMute", function() awful.spawn.with_shell("wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle") end, nil),
   awful.key({}, "XF86AudioMicMute", function() awful.spawn.with_shell("wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle") end, nil)
)
...
client.connect_signal(
   "request::titlebars", function(c)
      ...
      awful.titlebar(c, { size = 30, position = "top"}):setup {
         { -- Left
